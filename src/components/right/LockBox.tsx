...
const LockBox = ({ difficulty, numbers, setNumbers, isFlashing, flashColor }: LockBoxProps) => {
  ...
  return (
    <div className={`w-80 h-72 rounded-2xl flex items-center justify-center shadow-2xl border-4 border-white/20 backdrop-blur-md overflow-hidden ${boxGradients[difficulty]}`}>
      <div className="flex w-full h-full items-center justify-center">
        {numbers.map((num, idx) => (
          <div key={idx} className="flex flex-col items-center mx-4">
            ...
            <span className={`text-6xl font-mono text-white drop-shadow-lg select-none px-4 py-2 rounded-lg border border-white/20 mb-3 transition-all duration-300 ${
